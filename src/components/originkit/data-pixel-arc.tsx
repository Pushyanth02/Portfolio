...
/**
 * DataPixelArc — a hard pixel mosaic cut into an arc.
 *
 * Sibling to PredictiveArc and deliberately not the same thing: the dots here
 * are FIXED-SIZE squares with a gutter and the intensity rides their opacity,
 * where PredictiveArc modulates the square's side and leaves opacity alone. That
 * one difference is the whole character — a crisp readout against a soft cloud.
 *
 * Two other things the curve does differently, and both matter more than the
 * palette: the arc is |x|^1.8 rather than x^2, which is flatter across the top
 * and falls away harder at the shoulders; and the wave is ADDED to the intensity
 * rather than multiplied by it, so it can push cells above the band's own
 * envelope and scatter live pixels beyond the edge instead of merely dimming the
 * ones already inside it.
 *
 * The port drops the Canvas 2D double loop (a per-cell field is what a fragment
 * shader evaluates for free), the `dark`/`light` enum (the four colour dials
 * express both, and `mix(background, ink, coverage)` composites either way), and
 * the `hue`/`saturation` CSS filter over the canvas, which only rotated the
 * colours already chosen in the panel.
 *
 * Interaction (asked for; the source has none): the cursor is a PROBE. Cells
 * within reach of it read out hotter, climbing the same intensity ramp the band
 * already uses, so the mosaic behaves like a readout being interrogated rather
 * than a picture with a spotlight on it. Framer's editor swallows pointer events,
 * so it is PREVIEW-only; the resting frame is unchanged.
 *
 * Ported from the Predictive Arc collection.
 */
...
uniform vec2  uRes;
uniform float uTime, uDpr, uCell, uFill, uHover, uReach;
uniform vec2  uPtr;   // CSS px, y from the top
...
void main(){
  ...
  // A PROBE, not a moved curve: cells near the cursor read out hotter, so the
  // mosaic behaves like a readout being interrogated. (Its sibling PredictiveArc
  // moves the curve instead — one gesture each, not the same one twice.)
  float probe = (1.0 - smoothstep(0.0, 1.0, distance(vec2(x, y), uPtr) / max(uReach, 1.0))) * uHover;
  i = clamp(i + probe * 0.45, 0.0, 1.2);
...
export default function DataPixelArc(props: DataPixelArcProps) {
    ...
    const ptrRef = useRef({ tx: -1, ty: -1, x: -1, y: -1 })
    ...
    useEffect(() => {
        ...
        let clock = 0
        const PTR_RATE = 9.0 // 1/s, how fast the probe follows

        const render = (now: number) => {
            const dt = Math.min(0.05, (now - last) / 1000)
            last = now
            const v = vRef.current
            // the source advances 0.02 per frame at speed 1; expressed per second
            ...
            gl.uniform1f(u("uReach"), Math.min(bw, bh) / dpr * 0.28)
