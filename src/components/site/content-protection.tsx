...
/**
 * A hostname is authorized if it is listed verbatim OR if it lives under an
 * authorized suffix (e.g. "*.space-z.ai" covers every preview-chat-*.space-z.ai
 * subdomain the sandbox preview gateway proxies through).
 */
const AUTHORIZED_HOST_SUFFIXES = [".space-z.ai"];

const isAuthorizedHost = (hostname: string) =>
  AUTHORIZED_HOSTS.includes(hostname) ||
  AUTHORIZED_HOST_SUFFIXES.some((suffix) => hostname.endsWith(suffix));
...
export function ContentProtection() {
  ...
  useEffect(() => {
    ...
    if (!isAuthorizedHost(window.location.hostname)) {
