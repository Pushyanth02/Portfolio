 ...
 * tied to stable CSS selectors (`.reveal`, `.lm`, `.tilt`, `.hero-doodle`,
...
export function ClientEffects() {
  ...
  useEffect(() => {
    ...
    // --- hero doodle parallax (rAF batched) ---
    const doodle = document.querySelector<HTMLElement>(".hero-doodle");
    let doodleMove: ((e: MouseEvent) => void) | null = null;
    let doodleRaf = 0;
    if (doodle && !reduced) {
      let mouseX = 0;
      let mouseY = 0;
      ...
      const renderDoodle = () => {
        ...
        doodle.style.transform = `rotate(3deg) translate(${(x * 10).toFixed(1)}px, ${(y * 8).toFixed(1)}px)`;
      };

      doodleMove = (e: MouseEvent) => {
        mouseX = e.clientX;
        mouseY = e.clientY;
        if (!doodleRaf) {
          doodleRaf = requestAnimationFrame(renderDoodle);
        }
      };
      window.addEventListener("mousemove", doodleMove, { passive: true });
    }
    ...
    return () => {
      ...
      if (doodleMove) window.removeEventListener("mousemove", doodleMove);
      if (doodleRaf) cancelAnimationFrame(doodleRaf);
    };
  }, []);
